...
<%
	...
	{
%>
<tr><td><%= m.getDid() %></td><td><%= m.getTitle() %></td><td><%= m.getBookdate() %></td><td><%= m.getCustomerName() %></td><td><%= m.getCustomerEmail() %></td><td><%= m.getStatus() %></td><td><a href="ChangeStatus.jsp?email=<%= m.getCustomerEmail()%>&name=<%= m.getCustomerName()%>&bdate=<%= m.getBookdate()%>">To Change status click here</a></td></tr>
<% } %>
</table>
<% }
	else
